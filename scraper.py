...
async def run_scraper():
    ...
    # 1. جلب البيانات القديمة للدمج (عشان نضمن الـ Unique من الأول)
    try:
                    ...
                    for link in raw_matches:
                        ...
                        if len(clean_user) > 4:
                            new_results.append({
                                ...
                                "Subscribers": "N/A", # كما اتفقنا لتسريع العملية
                                "LatestID": None
                            })
    ...
    # 3. الدمج النهائي والتنظيف (The Smart Clean)
    # تحويل النتائج الجديدة لـ DataFrame
    new_df = pd.DataFrame(new_results)
    
    # دمج [القديم 1 + القديم 2 + الجديد المستخرج]
    final_df = pd.concat([existing_df, new_df], ignore_index=True)
    
    # التأكد من ترتيب العواميد الـ 5 اللي طلبتها
    ...
    # مسح التكرار بناءً على الـ Link (الحفاظ على البيانات اللي فيها LatestID لو وجدت)
    # بنرتب بحيث الـ LatestID اللي مش فاضي يجي في الأول وبعدين نمسح التكرار
